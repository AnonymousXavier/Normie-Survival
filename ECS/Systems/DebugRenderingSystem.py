...
def process(surface: pygame.Surface, debug: dict, debug_grid: dict, camera: dict, real_world: dict = None):
    ...
    # 1. Create a SINGLE master surface for all debug info
    # This surface is in World Pixels
    master_debug_surf = pygame.Surface(cam_boundary["world_size"], pygame.SRCALPHA)

    # 2. Draw Hitboxes onto the master surface
    if real_world:
        for obj in real_world.values():
            if HitboxComponent in obj and SpacialComponent in obj:
                ...
                # Relative to camera
                render_pos = h_rect.left - camera_rect.left, h_rect.top - camera_rect.top
                render_rect = pygame.Rect(render_pos, h_rect.size)
                
                # FIX: Use width=2 so lines don't disappear when scaled
                pygame.draw.rect(master_debug_surf, Settings.DEBUG.HITBOX_COLOR, render_rect, 2)

    # 3. Draw Arrows onto the SAME master surface
    # We pass the master_debug_surf into the helper
    draw_debug_elements(master_debug_surf, debug, debug_grid, camera, cam_boundary)

    # 4. SCALE ONCE AND BLIT
    # This is much more efficient and prevents vanishing lines
    transformed_debug = pygame.transform.scale(master_debug_surf, rendering_data["size"])
    surface.blit(transformed_debug, rendering_data["offset"])

def draw_debug_elements(render_surface: pygame.Surface, debug: dict, debug_grid: dict, camera: dict, cam_boundary: dict):
