from ECS.Components import AnimationComponent, VelocityComponent, FacingDirectionComponent
from Globals import Enums

def process(world: dict):
    for obj in world.values():
        # Check if the entity is capable of animating and facing directions
        if AnimationComponent in obj and FacingDirectionComponent in obj:
            ...
            # 1. Track if the state is changing this frame
            old_state = anim.state

            # 2. Determine State (Walking or Idle?)
            # In your architecture, entities only have a VelocityComponent while actively moving
            if VelocityComponent in obj:
                anim.state = Enums.ANIM_STATES.WALK
            else:
                anim.state = Enums.ANIM_STATES.IDLE

            # 3. Reset the frame if they just started or stopped moving
            if old_state != anim.state:
                anim.current_frame = 0.0

            # 4. Determine Direction based on the Enum mappings
            if facing.dx == 1:
                anim.direction = Enums.ANIM_DIRS.RIGHT
            elif facing.dx == -1:
                anim.direction = Enums.ANIM_DIRS.LEFT
            elif facing.dy == 1: # Positive Y is Down in Pygame
