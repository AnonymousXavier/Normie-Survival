            ...
            for cell in neighbor_cells:
                if cell in spatial_grid:
                    for target_id in list(spatial_grid[cell]):
                        # Collision logic
                        if (
                            EnemyTag in world[target_id]
                            and HealthComponent in world[target_id]
                        ):
                            enemy_rect = world[target_id][SpacialComponent].rect

                            if proj_rect.colliderect(enemy_rect):
                                # Apply damage
                                world[target_id][HealthComponent].hp -= world[proj_id][
                                    ProjectileComponent
                                ].damage
                                ...
                                if world[target_id][HealthComponent].hp <= 0:
                                    ...
                                    for _ in range(num_gems):
                                        ...
                                        Factories.spawn_gem(
                                            world, spatial_grid, rx, ry, value=gem_value
                                        )
